<template>
  <!-- 年龄比例 -->
  <div class="echarts">
    <ECharts :option="options" :resize="false" />
  </div>
</template>
...
let data: any = null;
function initItems() {
  items = orignData.items.map(function (el: any, index: any) {
    return {
      name: el.name,
  ...
  series: [
    ...
    },
    {
      ...
      label: {
        ...
        color: "#ffffff", //此行代码为了解决labelformater字体描边问题
        show: true
        // }
      },
      hoverAnimation: true,
...
onMounted(() => {
  ...
  options.value.series[0].links = links;
  options.value.series[1].data = linesData;
  options.value.series[2].data = scatterData;
});
</script>
